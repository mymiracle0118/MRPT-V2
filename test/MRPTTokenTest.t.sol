...
contract MRPTTokenTest is Test {
    ...
    //     // console.log(IVestingWallet(mrptToken.advisorsAddr()).vestedAmount(address(mrptToken),
    //     // uint64(block.timestamp)));

    //     uint256 start_timestamp = vm.envUint("START_TIMESTAMP");

    //     mrptToken = new MockMRPTToken(uint64(start_timestamp), ecoSystem, marketing, stakingRewards, team, advisors);
    ...
    //     assertEq(IVestingWallet(mrptToken.marketingAddr()).beneficiary(), marketing);
    //     assertEq(IVestingWallet(mrptToken.marketingAddr()).duration(), 20 * 30 days);
    //     assertEq(mrptToken.balanceOf(mrptToken.marketingAddr()), 900e24 * 0.18);

    //     assertEq(IVestingWallet(mrptToken.stakingRewardsAddr()).beneficiary(), stakingRewards);
    //     assertEq(IVestingWallet(mrptToken.stakingRewardsAddr()).duration(), 60 * 30 days);
    //     assertEq(mrptToken.balanceOf(mrptToken.stakingRewardsAddr()), 900e24 * 0.21);
    ...
    //     assertEq(IVestingWallet(mrptToken.advisorsAddr()).beneficiary(), advisors);
    //     assertEq(IVestingWallet(mrptToken.advisorsAddr()).duration(), 20 * 30 days);
    ...
    //     // console.log(IVestingWallet(mrptToken.ecoSystemAddr()).vestedAmount(address(mrptToken),
    ...
    function test_transferFee() public {
        ...
        assertEq(mrptToken.balanceOf(alice), 10 ether);
