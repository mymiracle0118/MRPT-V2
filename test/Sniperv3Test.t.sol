...
contract SnperV2Test is Test {
    ...
    function test_buyMRPT() public {
        ...
        vm.prank(alice);
        (bool success,) =
            address(sniperV2).call{ value: 1 ether }(abi.encodeWithSignature("buyToken(uint256)", 1 ether));
        if (!success) return;
    ...
    function test_WithdrawToken() public {
        ...
        vm.prank(alice);
        (bool success,) =
            address(sniperV2).call{ value: 2 ether }(abi.encodeWithSignature("buyToken(uint256)", 1 ether));
        if (!success) return;
    ...
    function test_createAndAddLiquidity() public {
        ...
        vm.prank(alice);
        (bool success,) =
            wethAddress.call{ value: 10 ether }(abi.encodeWithSignature("deposit(address,uint256)", alice, 10 ether));
        if (!success) return;
        ...
        IUniswapV2Router02(routerAddress).addLiquidity(
            address(mrptToken), wethAddress, 10 ether, 10 ether, 9 ether, 9 ether, alice, block.timestamp
        );
    }
}
